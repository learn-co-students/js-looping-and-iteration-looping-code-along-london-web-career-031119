...
describe("index.js", () => {
  ...
  describe("tailsNeverFails()", () => {
    ...
    it('loops indefinitely until a flip results in "Heads"', () => {
      spy.onCall(0).returns(0.7);
      spy();
      spy.onCall(1).returns(0.6);
      spy();

      spy.onCall(2).returns(0.5);
      spy();

      spy.onCall(3).returns(0.9);
      spy();

      spy.onCall(4).returns(0.8);
      spy();

      spy.onCall(5).returns(0.5);
      spy();

      spy.onCall(6).returns(0.7);
      spy();

      spy.onCall(7).returns(0.6);
      spy();

      spy.onCall(8).returns(0.4);
      spy();
